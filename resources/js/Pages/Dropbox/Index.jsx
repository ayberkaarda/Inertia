    ...
    const handleUpload = (e) => {
        ...
        post(route('dropbox.store', selectedSprint.id), {
            ...
            onSuccess: () => {
                ...
                // Mevcut seçili sprintin dosyalarını güncellemek için state'i manuel yenileyebiliriz 
                // ama Inertia zaten sayfayı yenileyecek.
                        ...
                        {sprints.length === 0 ? (
                            ...
                            <div className="flex flex-col gap-2">
                                {sprints.map((sprint) => (
                                    ...
                                    >
                                        <span className={`text-sm font-bold ${selectedSprint?.id === sprint.id ? 'text-purple-300' : 'text-slate-300'}`}>
                                            {sprint.name}
                                        </span>
                ...
                <div className="w-full lg:w-2/3 flex flex-col gap-6">
                    {selectedSprint ? (
                        <>
                            {/* DOSYA YÜKLEME ALANI */}
                            <div className="bg-[#160d33]/80 backdrop-blur-xl p-6 rounded-3xl border border-purple-500/20 shadow-2xl">
                                <h3 className="text-white font-black text-sm tracking-widest uppercase mb-4">
                                    Upload to {selectedSprint.name}
                                </h3>
                                ...
                                <form onSubmit={handleUpload} className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                                    <div className="flex-1 w-full relative">
                                        ...
                                        <label 
                                            htmlFor="file-upload"
                                            className="w-full flex items-center justify-center gap-2 p-4 border-2 border-dashed border-purple-500/30 rounded-xl hover:bg-purple-500/10 hover:border-purple-500 cursor-pointer transition-all text-sm text-slate-300 font-bold"
                                        >
                                            <svg className="w-5 h-5 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                                    ...
                                    <button 
                                        ...
                                        className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-black text-xs uppercase tracking-widest rounded-xl disabled:opacity-50 transition-all shadow-[0_0_15px_rgba(168,85,247,0.3)] shrink-0 h-[56px]"
                                    >
                            ...
                            <div className="bg-[#160d33]/80 backdrop-blur-xl p-6 rounded-3xl border border-purple-500/20 shadow-2xl flex-1">
                                ...
                                <div className="flex flex-col gap-3">
                                    {/* O an seçili olan sprintin güncel halini sprints içinden bul */}
                                    {sprints.find(s => s.id === selectedSprint.id)?.files?.length > 0 ? (
                                        sprints.find(s => s.id === selectedSprint.id).files.map((file) => (
                                            <div key={file.id} className="group flex items-center justify-between p-4 bg-[#0d0722]/50 border border-white/5 hover:border-purple-500/30 rounded-xl transition-all">
                                                ...
                                                {/* Silme Butonu (Sadece yükleyen kişiyse) */}
                        ...
                        <div className="bg-[#160d33]/80 backdrop-blur-xl p-10 rounded-3xl border border-white/5 shadow-2xl flex flex-col items-center justify-center text-center h-full min-h-[400px]">
                            ...
                            <h3 className="text-white font-black text-lg tracking-widest uppercase mb-2">Vault Locked</h3>
