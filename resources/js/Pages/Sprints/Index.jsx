...
export default function SprintsIndex({ 
    ...
    const handleToggleCompletion = (task, sprintStatus) => {
        // Eğer sprint zaten bittiyse veya task tamamsa işlem yapma
    ...
    const toggleStatus = (id, currentStatus) => {
        ...
        const confirmMsg = nextStatus === 'completed' 
            ? "Completing the sprint will lock all missions inside. Proceed?" 
            : `Set sprint to ${nextStatus}?`;
            ...
            <div className="bg-[#160d33]/80 backdrop-blur-xl border border-purple-500/30 rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-8">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
                    {initialSprints.map((sprint) => {
                        ...
                        return (
                            <div key={sprint.id} className={`bg-[#0f0822] border rounded-2xl p-4 sm:p-6 shadow-lg flex flex-col transition-all ${isSprintOver ? 'border-slate-700/50 bg-[#0a0516]' : 'border-purple-500/20 hover:border-purple-500/40'}`}>
                                ...
                                <div className="space-y-2 mb-4 flex-1 overflow-y-auto max-h-[250px] sm:max-h-[300px] pr-1 scrollbar-thin scrollbar-thumb-purple-500/20">
                                    {(sprint.tasks || []).map(task => {
                                        ...
                                        // Task, sprint bittiyse veya kendi bittiyse kilitlidir
                                        const isLocked = task.is_completed || isSprintOver;

                                        return (
                                            <div key={task.id} className={`flex justify-between items-center bg-[#1a0b2e] border p-2 sm:p-3 rounded-xl transition-all ${isLocked ? 'opacity-40 grayscale border-slate-800' : 'border-purple-500/10 hover:border-purple-500/30'}`}>
