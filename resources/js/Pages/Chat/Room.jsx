...
export default function Room({ auth, conversation, messages, receiver }) {
    ...
    // 🌟 Odaya ilk girildiğinde okunmamış mesajları okundu olarak işaretle
    const triggerMarkAsRead = async () => {
        try {
            await axios.post(`/chat/${conversation.id}/read`);
        } catch (error) {
            console.error("Read status could not be updated:", error);
        }
    };

    useEffect(() => {
        // Oda ilk açıldığında backend'e "Okudum" bildirimi at
        triggerMarkAsRead();

        // 🌟 REAL-TIME KANAL DİNLEYİCİLERİ
        window.Echo.private(`chat.${conversation.id}`)
            .listen('MessageSent', (e) => {
                if (e.message.sender_id !== auth.user.id) {
                    setMessagesList((prev) => [...prev, e.message]);
                    // Eğer ben şu an bu odadaysam ve karşıdan mesaj geldiyse anında okundu yap
                    triggerMarkAsRead();
                }
            })
            // 🌟 Görüldü Sinyali Geldiğinde Tüm Mesajları Ön Yüzde "Okundu" Yap
    ...
    const sendMessage = async (e) => {
        ...
        try {
            ...
            // Backend'den dönen yeni mesajı listeye ekle
                    ...
                    <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4 sm:space-y-5 z-10 scrollbar-thin scrollbar-thumb-purple-500/30">
                        {messagesList.length === 0 ? (
                            ...
                            messagesList.map((msg) => {
                                ...
                                // 🌟 GÖRÜLDÜ DURUMU: Mesajda read_at varsa okundu sayılır
                                ...
                                return (
                                    <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                        ...
                                        <span className="text-[9px] sm:text-[10px] text-slate-500 mt-1 sm:mt-1.5 font-bold tracking-widest uppercase flex items-center gap-1">
                                            ...
                                            {isMe && (
                                                isRead ? (
                                                    // Görüldü: Parlayan Mor Çift Tik
                                                    <span className="text-purple-400 font-black tracking-tighter drop-shadow-[0_0_5px_rgba(168,85,247,0.8)]">✓✓</span>
                                                ) : (
                                                    // İletildi Ama Okunmadı: Sönük Gri Çift Tik
                                                    <span className="text-slate-600 font-black tracking-tighter">✓✓</span>
                                                )
                                            )}
                                        </span>
                                    </div>
